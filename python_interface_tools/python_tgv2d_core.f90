...
contains
  ...
  ! Add two new input arguments to the subroutine: python_case_name and python_cps
  subroutine run_tgv2d(par_env, shared_env, error_L2, error_Linf, input_dt, input_num_steps, python_case_name, python_cps)
    ...
    ! Define new input arguments
    character(len=:), allocatable, intent(in) :: python_case_name !< name of the case
    integer(ccs_int), intent(in):: python_cps   !< number of cells per side

    ! Define local variables
    ! input_mesh changed from input argument to local variable
    type(ccs_mesh), allocatable :: input_mesh !< mesh object to use, if not provided, the build_square_mesh is used
    ...
    ! Remove the call to read_command_line_arguments and replace it read from the input arguments
    ! call read_command_line_arguments(par_env, cps, case_name=case_name, in_dir=input_path)
    ! Assign the input arguments to the local variables
    case_name = python_case_name
    cps = python_cps
